...
const ProgramQuiz = () => {
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex items-center justify-center gap-4 sm:gap-6 md:gap-8 mb-8 md:mb-10">
              ...
              <div className="text-center">
                <div className="text-xl sm:text-2xl font-bold text-blue-400">₹15K</div>
                <div className="text-xs sm:text-sm text-gray-500">bonus value</div>
              </div>
            </div>
            ...
            <p className="mt-4 md:mt-6 text-sm md:text-base text-gray-500">
              ✓ No payment required • ✓ Instant results • ✓ ₹15K bonus
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="py-8 sm:py-16 md:py-20 relative overflow-hidden bg-slate-900/50">
      {/* Fixed progress bar at top on mobile */}
      <div className="fixed top-0 left-0 right-0 h-1 bg-gray-800 z-50 md:hidden">
        <div 
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-6 md:mb-8">
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm md:text-base text-gray-400">Question {currentQuestion + 1} of {questions.length}</span>
              <span className="text-sm md:text-base font-medium text-green-400">{Math.round(progress)}% complete</span>
            </div>
            
            {/* Visual progress indicators - Desktop only */}
          ...
          {/* Question - Mobile optimized padding */}
          <div className="glass-effect rounded-2xl p-4 sm:p-6 md:p-8 border border-white/10">
            {currentQuestion > 0 && (
              <button
                onClick={goBack}
            ...
            <div className="text-center mb-6 md:mb-8">
              ...
              {currentQ.subtitle && (
                <p className="text-gray-400 text-sm md:text-base">{currentQ.subtitle}</p>
              )}
            </div>
            ...
            <div className="grid gap-3">
              {currentQ.options.map((option) => (
                <button
                  ...
                  className={`relative overflow-hidden rounded-xl border border-white/10 hover:border-white/30 active:border-green-500/50 transition-all duration-300 group hover:scale-[1.02] active:scale-[0.98] hover:shadow-xl`}
                  style={{ minHeight: '80px' }} // Ensure mobile touch target
                >
                  <div className={`absolute inset-0 bg-gradient-to-r ${option.color} opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-opacity duration-300`} />
                  <div className="relative p-4 md:p-5 flex items-center gap-3 md:gap-4">
                    <div className="text-2xl sm:text-3xl md:text-4xl flex-shrink-0">{option.emoji}</div>
                    <div className="flex-1 text-left">
                      <div className="font-semibold text-white text-base md:text-lg mb-1">{option.label}</div>
                      {option.description && (
                        <div className="text-sm md:text-base text-gray-400 group-hover:text-gray-300">{option.description}</div>
                      )}
                    </div>
                    <ArrowRight className="w-5 h-5 text-gray-600 group-hover:text-white opacity-0 group-hover:opacity-100 md:block hidden transition-all transform group-hover:translate-x-1" />
                  </div>
                </button>
              ))}
            </div>
